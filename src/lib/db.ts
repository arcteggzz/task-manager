...
export async function updateProject(
  id: string,
  updates: Partial<
    Pick<Project, "name" | "description" | "status" | "isQuickAccess">
  >,
) {
  ensureDB();
...
export async function createTask(
  ...
  const nextIndex = tasksSnap.exists()
    ? Object.keys(tasksSnap.val()).length + 1
    : 1;
  const id = crypto.randomUUID();
  const task: Task = {
    id,
...
export async function updateTask(
  projectId: string,
  id: string,
  updates: Partial<Pick<Task, "name" | "description" | "status">>,
) {
  ensureDB();
...
export function subscribePendingTaskCount(
  callback: (count: number) => void,
): () => void {
  ensureDB();
  const unsubscribe = onValue(ref(db!, "tasks"), (snap) => {
    if (!snap.exists()) {
      callback(0);
      return;
    }
    let count = 0;
    const projectsData = snap.val() as Record<string, Record<string, Task>>;
    for (const tasks of Object.values(projectsData)) {
